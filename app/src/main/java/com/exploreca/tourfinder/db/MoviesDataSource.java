...
//        contentValues.put(MovieDBOpenHelper.COLUMN_VIDEO, (movie.getVideo())== true ? 1 : 0);
    ...
    public void deleteMovies(){
        ...
        for(Movie movie: movies_orig){
            name = movie;
            continue;
        }
       // String query =  name.getTitle().toString();
        sqLiteDatabase.delete(MovieDBOpenHelper.TABLE_MY_MOVIES, MovieDBOpenHelper.COLUMN_TITLE + " = ?", new String[] { String.valueOf(name.getTitle())});

        //String query = " DELETE FROM " + MovieDBOpenHelper.TABLE_MY_MOVIES + " WHERE COLUMN_TITLE = ";
